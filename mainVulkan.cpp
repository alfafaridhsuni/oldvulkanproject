...
const std::vector<const char *> deviceExtensions = {
    ...
    "VK_KHR_portability_subset" // ✅ REQUIRED for MoltenVK/macOS
    //"VK_KHR_get_physical_device_properties2"  // ✅ Required dependency
};
...
private:
    ...
    void pickPhysicalDevice() {
        ...
        for (const auto& device : devices) {
            if (isDeviceSuitable(device)) {
                physicalDevice = device;    // Select a physical device.
		msaaSample = getMaxUsableSampleCount();
                break;
            }
        }
	...
	subpass.pResolveAttachments = &colorAttachmentResolveRef;
        ...
        if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) {
            throw std::runtime_error("failed tp create render pass!");
        }
    }
	...
	std::cout << "Successfuly load obj model: " << MODEL_PATH.c_str() << "\n"; 
...
//        stbi_uc *pixels = stbi_load("textures/texture.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	mipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(texWidth, texHeight)))) + 1;
        ...
        std::cout << "Succesfully load texture image: " << TEXTURE_PATH.c_str() << "\n";
	...
	// transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
	...
	int32_t mipWidth = texWidth;
        ...
        for (uint32_t i = 1; i < mipLevels; i++) {
            ...
            vkCmdPipelineBarrier(commandBuffers,
...
//	samplerInfo.minLod = static_cast<float>(mipLevels / 2);
    ...
    void createCommandPool() {
        ...
        if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool) != VK_SUCCESS) {
            throw std::runtime_error("failed tp create command pool!");
        }
    }
    ...
    void createDepthResources() {
        ...
        createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSample,  depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
            ...
            //vkCmdDraw(commandBuffer, static_cast<uint32_t>(vertices.size()), 1, 0, 0);   // Without indexBuffer
